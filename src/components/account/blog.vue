<template>
  <div>
    <div class="card mt-3">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <span class="text-black-50">Blog Table</span>
          <b-button
            v-b-modal.modal-blog
            variant="outline-secondary"
            size="sm"
          >
            <i class="fas fa-plus mb-0"></i>
          </b-button>
        </div>
      </div>
      <div class="card-body">
        <table class="table align-middle table-responsive z-index-1000">
          <thead>
            <tr>
              <th>#</th>
              <th>title</th>
              <th>Image</th>
              <th>desc</th>
              <th>date</th>
              <th>Handle</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>1</td>
              <td>title</td>
              <td>
                <img
                  src="../../assets/img/tzuyu/1.jpg"
                  class="memo-img shadow-sm rounded-2"
                  alt=""
                />
              </td>
              <td>
                  desc
              </td>
              <td>
                  5.3.2000
              </td>
              <td>
                <b-dropdown variant="link" no-caret>
                  <template #button-content>
                    <i class="fas fa-list text-dark"></i>
                  </template>

                  <b-dropdown-item>
                    <i class="fas fa-edit text-black-50"></i>
                    <span
                      class="align-middle ml-50 text-black-50 p-2"
                      style="font-size: 15px"
                      >edit</span
                    >
                  </b-dropdown-item>

                  <b-dropdown-item>
                    <i class="fas fa-trash text-black-50"></i>
                    <span
                      class="align-middle ml-50 text-black-50 p-2"
                      style="font-size: 15px"
                      >delete</span
                    >
                  </b-dropdown-item>
                </b-dropdown>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>

<!--    Modal -->
    <b-modal id="modal-blog" no-close-on-backdrop centered title="Blog" hide-footer >
      <form @submit.prevent="create()">
        <div class="input-group">
          <input type="text" placeholder="title" v-model="Form.title" class="form-control" required>
        </div>
        <div class="input-group mt-3">
            <textarea name="" id="" cols="5" rows="5" class="form-control" v-model="Form.desc" placeholder="description"></textarea>
        </div>
        <div class="input-group mt-3">
          <div class="border border-1 border-secondary p-5 rounded-3 ">
            <i class="fas fa-plus"></i>
          </div>
          <input type="file" id="imageUpload" class="form-control d-none">
        </div>
        <div class="input-group mt-3">
          <!-- <img src="../../assets/img/tzuyu/1.jpg" class="img-fluid border border-1 border-light rounded-2 shadow-sm" height="50px" alt=""> -->
        </div>
        <div class="input-group mt-3">
          <button class="btn btn-outline-dark " >Create</button>
        </div>
      </form>
    </b-modal>
<!--    Modal -->

  </div>
</template>

<script>
    export default {
        data() {
            return {
                Form:{
                    'title' : '',
                    'image' : '',
                    'desc'  : '',
                    'date'  : '',
                }
            }
        },
        computed: {
        today() {
                let today = new Date();
                let dd = String(today.getDate()).padStart(2, "0");
                let mm = String(today.getMonth() + 1).padStart(2, "0"); //January is 0!
                let yyyy = today.getFullYear();
                return mm + "/" + dd + "/" + yyyy;
            }
        },
        methods: {
            create() {
            
                console.log(this.Form)
            }
        },
    }
</script>

<style lang="scss" scoped>

</style>